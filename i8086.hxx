...
// The 8086 can address ffff:ffff, which is 0..0x10ffef.
// But it only has 20 address lines, so high memory area (HMA) >= 0x100000 references wrap to conventional memory.
// By default, don't wrap since that has a 9.4% performance penalty and just apps made by IBM Pascal 2 depends on it.
// Microsoft LISP v5.10 (mulisp.exe) references HMA and works whether wrapping or not.

#define WRAP_HMA_ADDRESSES false           // set to true to get apps generated by IBM Pascal 2 to run
