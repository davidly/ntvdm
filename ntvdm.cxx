...
// Tested with Wordstar Release 4 and GWBASIC.COM.
//
...
//     0x00800 -- 0x00bff   where I put the environment
//     0x00c00 -- 0x00fff   interrupt routines
//     0x01000 -- 0x01fff   unused (4k at 4k)
//     0x02000 -- 0x9ffff   apps are loaded here
//     0xa0000 -- 0xeffff   reserved for hardware
//     0xf0000 -- 0xfbfff   system monitor (0 for now)
//     0xfc000 -- 0xfffff   bios code and hard-coded bios data (0 for now)
...
const char * get_interrupt_string( byte i, byte c )
{
...
void InitializePSP( WORD segment, char * acAppArgs, const char * pcAPP )
{
    ...
    * pargs = len;
    ...
    const WORD EnvironmentSegment = 0x80; 
    * (WORD *)  ( GetMem( segment, 0x2c ) ) = EnvironmentSegment;
    byte * penvdata = GetMem( EnvironmentSegment, 0 );
    penvdata[ 0 ] = 0; // no environment variables
    penvdata[ 1 ] = 0; // no environment variables
    * (WORD *)  ( penvdata + 2 ) = 0x0001; // one more additional item per DOS 3.0+
    GetFullPathNameA( pcAPP, 256, (char *) penvdata + 4, 0 );
...
int main( int argc, char ** argv )
{
    ...
    * ( pbiosdata + 0x10f ) = 0;
    ...
    uint8_t * pRoutines = (byte *) GetMem( 0xc0, 0 ); // that's address 0xc00
