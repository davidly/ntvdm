...
// and now in May 2025 I see Microsoft regressed yet more of the characters including 0x1e and 0x1f.
...
static const wchar_t CP437_to_Unicode_Windows_Hack[ 32 ] =
{
    ...
    0x2265, 0x2264, 0x007c, 0x203c, 0x00b6, 0x00a7, 0x25ac, 0x007c, 0x007c, 0x007c, 0x002d, 0x002d, 0x221f, 0x2194, 0x007c, 0x007c,
};
...
bool starts_with( const char * str, const char * start )
{
    ...
    bool ProcessFoundFileFCB( WIN32_FIND_DATAA & fd, uint8_t attr, bool exFCB )
    {
        ...
        {
            pdot++;
            for ( int i = 0; i < _countof( pfcb->ext ) && 0 != pdot[ i ]; i++ )
                pfcb->ext[i] = pdot[ i ];
        }
...
bool CheckForIntelC45App( FILE * fp )
{
    ...
    if ( file_size > 1000 )
    {
        if ( -1 != fseek( fp, file_size - 16, SEEK_SET ) )
        {
            char ac8[ 16 ] = {0};
            if ( fread( ac8, 1, 8, fp ) )
...
void ValidateStateLooksOK()
{
    ...
    // Zortech C v2 and v3 free RAM that its stack pointer is using
    ...
    {
        DosAllocation & da = g_allocEntries[i];
        // too aggressive since the initial process may have hooked interrupts. if ( da.seg_process == g_currentPSP || da.seg_process == segParent )
        // for example: the Microsoft Cobol compiler v3, v4.5, and v5
        {
    ...
    if ( !codeInRange )
    {
        tracer.Trace( "validation error: cs:ip is out of range\n" );
        trace_all_allocations();
        cpu.trace_state();
        ...
        tracer.Trace( "g_currentPSP %04x, parent psp %04x\n", g_currentPSP, segParent );
    ...
    // stacks can be incorrect while interrupts are disabled since it's two instructions to update sp and ss

    if ( !stackInRange && cpu.get_interrupt() )
    {
        trace_all_allocations();
        cpu.trace_state();
        ...
        do
        {
            ...
            *pDailyTimer = GetBiosDailyTimer(); // apps look here even if no timer interrupts happen because they aren't hooked
            ...
            if ( cpu.get_interrupt() && !cpu.get_trap() )
            {
                ...
                // don't plumb through port 60 since most apps work without that. (not quick basic 2 though)
