...
// Only CGA text modes 2 and 3 (greyscale and color 80x25) are supported.
...
// Tested with Brief 3.1. Use b.exe's -k flag for compatible keyboard handling.
// I went from 8085/6800/Z80 machines to Amigas to IBM 360/370 to VAX/VMS, to Unix to
// Windows to NT and mostly skipped DOS programming. Hence this is filling a gap in my knowledge.
//
...
void InitializePSP( uint16_t segment, char * acAppArgs, const char * pcAPP )
{
    ...
    * (uint16_t *)  ( GetMem( segment, 0x2c ) ) = EnvironmentSegment;
    uint8_t * penvdata = GetMem( EnvironmentSegment, 0 );
    penvdata[ 0 ] = 0; // no environment variables
    penvdata[ 1 ] = 0; // no environment variables
    * (uint16_t *)  ( penvdata + 2 ) = 0x0001; // one more additional item per DOS 3.0+
...
int main( int argc, char ** argv )
{
    ...
    * ( pbiosdata + 0x10f ) = 0; // already 0, but ok
    ...
    * (uint8_t *) ( GetMem( 0xffff, 0xe ) ) = 0x69; // machine ID. nice.
    ...
    else
    {
        ...
        const uint16_t DataSegment = DataSegmentOffset / 16;
